...
var products = function() {
    ...
    /**
     * Constructs url for download of nomad file request to the endpoint
     * /download&date=[DATETIME]&time=[TIME]&level=[LEVEL] 
     * @param {[String]} path 
     */
    function constructDownloadUrl(path) {
        if (!path || !path[0]) return;

        var urlParts = path[0].split('/'); 
        console.log(urlParts);

        if (!urlParts || !urlParts[3]) return; //url should always be in the format /data/weather/...

        if (urlParts[3] == 'current') {
            //TODO - create date time from current date
        } else {
